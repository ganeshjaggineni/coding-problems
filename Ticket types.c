...
organized the great 'WonderWorks Magic Show'. Renowned magicians were invited
to mystify and thrill the crowd with their world’s spectacular magic tricks. The Ticket
booking for the show started 2 days prior and there were different types of tickets
offered with different fare. The show organizers wanted to place a scanning machine
at the entrance of the venue for scrutiny. The machine will take the input of a
character denoting the various ticket types and displays the equivalent ticket type of
the given character.
...
char ticket;
scanf("%c",&ticket);
if(ticket == 'E' || ticket == 'e')
    printf("Early Bird Ticket");
else if (ticket == 'D' || ticket == 'd')
    printf("Discount Ticket");
else if (ticket == 'V' || ticket == 'v')
    printf("VIP Ticket");
else if (ticket == 'S' || ticket == 's')
    printf("Standard Ticket");
else if (ticket == 'C' || ticket == 'c')
    printf("Child Ticket");
return 0;
}
